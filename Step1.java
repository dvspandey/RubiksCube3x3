/*
	...
	> Never executed means == [Note: I think 'if(check(cube) == 4)' this if block is not usefull because Now we exit from check function so no use of 'break main;' we will exit before this statement executeing compleatly because of User-defined exception statement.] 
															[OR]
									[function will terminate before executing this statement completely, 
									because where-ever we call check function it will become recurcive call if it get improve cube [count is increase or one more number of white comes to make desy] so,
									Before we getting 'return answer' from check funtion it will for finding correct[or expected] result of step1 Class 
									and when check function get expected result of Step1 class it will take that perfect cube refrence and throws a user-defined exception with "cube refrence"
									and terminated its execution. 
									...
									So conclusion is every check function call will do these two thing:
										...
										> Or going to call again Main function with new improve cube if it get count is incremented]
										> else simply return count.
										
Updates:
=======
sideTopLayer
	> add else-if block on 28-dec-2020.
*/
class Step1{ 
	static int value = 0;
...
/*-----------------------------------------------------------------------------------------*/
	...
	*/
	static char[][][] sideTopLayer(char[][][] cube)throws MyException{ // updated on 28-dec-2020  --dvspandey 
